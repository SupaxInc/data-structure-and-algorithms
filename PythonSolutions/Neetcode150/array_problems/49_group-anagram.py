...
class MostReadableSolution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for s in strs:
            ...
            # For each character in string find what 0-index it lands on based on unicode (e.g. 99(c)-97(a) = 2)
            for char in s:
                idx = ord(char) - ord("a")
                count[idx] += 1
    ...
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for string in strs:  # O(n)
            ...
            for char in string:  # O(m) -> O(n * m)
                ...
                alphabetCount[ord(char) - asciiPos] = str(
                    int(alphabetCount[ord(char) - asciiPos]) + 1
                )
